...
object DeviceManager {
  ...
  final case class RequestAllTemperatures(
      requestId: Long,
      groupId: String,
      replyTo: ActorRef[ResponseAllTemperatures])
  final case class ResponseAllTemperatures(
      requestId: Long,
...
class DeviceManager(context: ActorContext[DeviceManager.Command])
  ...
  override def onMessage(msg: Command): Behavior[Command] = msg match {
    ...
    case DeviceGroupTerminated(groupId) =>
      context.log
        .info("Device group actor for {} has been terminated ", groupId)
...
object DeviceGroup {
  def apply(groupId: String): Behavior[Command] =
    Behaviors.setup[Command](context => new DeviceGroup(context, groupId))
  sealed trait Command
...
object Device {
  ...
  final case class RecordTemperature(requestId: Long,
                                     value: Double,
                                     replyTo: ActorRef[TemperatureRecoded])
      extends Command
  ...
  final case class ReadTemperature(requestId: Long,
      ...
      extends Command
  final case class RespondTemperature(requestId: Long,
                                      deviceId: String,
                                      value: Option[Double])
  ...
  def apply(deviceIdToActor: Map[String, ActorRef[Device.Command]],
    ...
    Behaviors.setup { context =>
      Behaviors.withTimers { timers =>
        new DeviceGroupQuery(deviceIdToActor,
                             requestId,
                             requester,
                             timeout,
                             context,
...
class DeviceGroupQuery(
  ...
  private val respondTemperatureAdapter = context
    .messageAdapter[Device.RespondTemperature](WrappedRespondTemperature.apply)
  ...
  private def onRespondTemperature(
      response: Device.RespondTemperature): Behavior[Command] = {
...
class IotSupervisor(context: ActorContext[Nothing])
    extends AbstractBehavior[Nothing] {
